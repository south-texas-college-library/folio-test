...
AS $$
    ...
    SELECT
        ...
        lc.name,
        jsonb_path_query_first(ins.jsonb, '$.contributors[*].name')::text,	
        hr.call_number,
        jsonb_extract_path_text(it.jsonb , 'barcode'),
        jsonb_extract_path_text(it.jsonb , 'status', 'name'),
        REGEXP_REPLACE(COALESCE(jsonb_path_query_first(it.jsonb, '$.notes[*] ? (@.itemNoteTypeId == "1fceb11c-7a89-49d6-8ef0-2a42c58556a2").note')::text, '0.00'), '^\$*', '$') AS "Price",
        mt.name,
        ...
        NULLIF(REGEXP_REPLACE(jsonb_path_query_array(ins.jsonb, '$.identifiers[*].value')::text, '\[|\]|"| :.*?\$\d+\.\d{2}', '', 'g'), ''),
        hl.name,
        il.name,
		TRANSLATE(jsonb_path_query_array(ins.jsonb, '$.subjects[*].value')::text, '[]"', ''),
        COALESCE(loans.checkouts, 0),
        COALESCE(loans.renewals, 0),
        jsonb_path_query_first(ins.jsonb, '$.publication[*].publisher')::text,
