...
AS $$
    ...
    WHERE
        ...
        AND CASE department
            WHEN 'Library' THEN (sub_type != 'All' AND COALESCE(hr.call_number, '') ~ '^[A-Z]{1,3}\s*[0-9]') AND hl.name !~* '(CLE|Open Lab)'
            WHEN 'CLE' THEN (sub_type != 'All' AND COALESCE(hr.call_number, '') !~ '^[A-Z]{1,3}\s*[0-9]') AND hl.name ~* '(CLE)'
            WHEN 'Open Lab' THEN (sub_type != 'All' AND COALESCE(hr.call_number, '') !~ '^[A-Z]{1,3}\s*[0-9]') AND hl.name ~* '(Open Lab)'
            ELSE TRUE
        END
