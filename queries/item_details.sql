--metadb:function item_details

DROP FUNCTION IF EXISTS item_details;

CREATE FUNCTION item_details(
...
RETURNS TABLE(
    ...
    "B - Call Number" TEXT,
    "C - Identifiers" TEXT,
    "D - Publication Date" TEXT,
	"E - Item Barcode" TEXT,
    "F - Holdings Location" TEXT,
    "G - Item Location" TEXT,
    "H - Material Type" TEXT,
    "I - Service Point" TEXT,
    "J - Item Status" TEXT,
    "K - Permanent Loan Type" TEXT,
    "L - Temporary Loan Type" TEXT,
    "M - Circulation Note" TEXT,
    "N - Public Note" TEXT,
    "O - Staff Notes" TEXT,
    "P - Ownership" TEXT,
    "Q - Price" TEXT,
    "R - Inventory Date" TEXT,
    "S - PO Number" TEXT,
    "T - Invoice" TEXT,
	"U - Subtype" TEXT,
    "V - Fund" TEXT
...
AS $$
    SELECT
        ...
        NULLIF(REGEXP_REPLACE(jsonb_path_query_array(it.jsonb, '$.notes[*] ? (@.itemNoteTypeId == "b6b35579-ee2b-4973-8e0d-ebc05bab0dab").note') #>> '{}', '[\[\]"]', '', 'g'), '') AS public_note,
        ...
        NULLIF(REGEXP_REPLACE(jsonb_path_query_array(it.jsonb, '$.notes[*] ? (@.itemNoteTypeId == "34207e4e-5cd7-4eab-801b-b0326cd5c66a").note') #>> '{}', '[\[\]"]', '', 'g'), '') AS ownership,
